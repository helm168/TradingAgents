...
PEG        40%   PE / 净利润增长率, GARP 核心. 增长率取自 G-Score 的净利 CAGR.
...
# financials parquet 拿 3Y CAGR 净利润增长.
def _net_income_cagr(ts_code: str) -> Optional[float]:
    """直接从 financials parquet 算 3Y CAGR (退化到 YoY).
    ...
    n = len(recent) - 1
    seq = recent["net_income"].astype(float).tolist()
...
def compute_vscore(ticker: str) -> Optional[VScoreResult]:
    ...
    pe = valuation["pe_ttm"]
    pb = valuation["pb"]
    growth = _net_income_cagr(ts_code)
